...
echo "=== [0/5] base deps (VM images are bare ubuntu)"
command -v python3 >/dev/null || apt-get update -qq
apt-get install -y -qq python3-pip python3-venv git curl >/dev/null 2>&1 || true
command -v pip >/dev/null || alias pip=pip3
...
cd ft-diloco
python3 -c "import torch" 2>/dev/null || pip3 install -q torch --index-url https://download.pytorch.org/whl/cu128
pip3 install -q -e ".[prep]" torchft-nightly==2026.6.10 --break-system-packages 2>/dev/null || pip3 install -q -e ".[prep]" torchft-nightly==2026.6.10

echo "=== [3/5] data (tokenize from HF — cloud pipe is fast)"
python3 -m ftdiloco.data --dataset tinystories --out data/tinystories
...
GLOO_SOCKET_IFNAME="${GLOO_IFACE:-tailscale0}" \
